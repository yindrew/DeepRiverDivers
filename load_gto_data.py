...
class GTOHandParser:
    ...
    def post_flop_sequencing(self, hero: Player, villain: Player) -> bool:
        ...
        """
        if hero == Player.UTG: 
            if villain == Player.UTG_PLUS_1 or villain == Player.UTG_PLUS_2 or villain == Player.DEALER:
                return True
            else:
                return False
    ...
    def parse_preflop_action(self, line: str) -> Optional[Tuple[Player, float, Action]]:
        """
        ...
        for i in range(0, len(parts), 2):
            temp_action = self.action_map[parts[i+1]] if parts[i+1] in self.action_map else Action.RAISE
    ...
    def parse_flop_action(self, line: str) -> Tuple[List[str], List[GameAction]]:
        ...
        for i in range(1, len(parts)):
            ...
            if temp_action == Action.CALL and previous_action == Action.RAISE: # if you are calling a raise, you need to factor in your original bet size.
                temp_amount = (float(parts[i-1]) - float(parts[i-2])) / self.pot_size
                ...
                temp_amount = float(parts[i]) / (self.pot_size + 2*float(parts[i-1]))
                self.pot_size += float(parts[i])
            ...
            actions.append(action)
        self.pot_size = temp_pot_size + (float(parts[-2]) * 2) if previous_action != Action.CHECK else self.pot_size
        return board, actions
    
    def parse_turn_or_river_action(self, line: str, current_street: Street ) -> Tuple[str,List[GameAction]]:
        ...
        for i in range(1, len(parts)):
            ...
            actions.append(action)
        self.pot_size = temp_pot_size + (float(parts[-2]) * 2) if Street.TURN == current_street else self.pot_size
        return board, actions    

    def parse_hand_file(self, file_path: str) -> List[Tuple[HandHistory, float]]:
        ...
        # Parse river
        river_card, river_actions = self.parse_turn_or_river_action(lines[5], Street.RIVER)
